{{ config(materialized='table', schema='arab_acmg_harmonized', tags=['t003', 'brca', 'gnomad', 'genomes']) }}
...
source_rows as (
    -- [AI-Agent: Codex]: Genomes and exomes share the same GRCh38 coordinate system.
    -- The scientific difference is cohort coverage, not position semantics.
    select
        ...
        gnomad.ac_eur_proxy,
        gnomad.af_eur_proxy
    from {{ ref('stg_gnomad_genomes_variants') }} as gnomad
    join gene_windows
