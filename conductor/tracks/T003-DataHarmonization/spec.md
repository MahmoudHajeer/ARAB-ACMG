...
- De-identified Arab cohort extracts frozen to GCS after source-package intake.
...
- no downstream BigQuery dependency; harmonized outputs must be buildable from GCS-hosted raw/frozen source packages alone.
...
- Metadata contract:
  - every harmonized row must carry the transformation metadata fields defined in [conductor/data-contracts.md](../../data-contracts.md)
  - spreadsheet-derived sources must preserve `source_sheet_name` and `source_row_number`
...
- Quality gates:
  ...
  - DuckDB-backed validation checks and/or `pytest` assertions to enforce canonical key invariants without restoring durable BigQuery harmonized tables

## Success Criteria
- [ ] All harmonized artifacts use the canonical GRCh38 `variant_key`.
- [ ] All harmonized artifacts expose the required transformation metadata contract (`parse_status`, `liftover_status`, `norm_status`, tool/version provenance).
